package com.baizeli.eternisstarrysky.cora;
...
public class LNGPlusLaunchPluginService implements ILaunchPluginService {

    private static final String owner = "com/baizeli/eternisstarrysky/cora/utils/EventUtil";
    @Override
    public String name() {
        return "LNGLaunchPluginService";
    }
    ...
    public boolean processClass(Phase phase, ClassNode classNode, Type classType) {
        if (classNode.name.startsWith("com/baizeli/eternisstarrysky")) return false;
        boolean flag = false;
        if (classNode.name.equals("net/minecraft/world/entity/LivingEntity") || is(classNode, "net/minecraft/world/entity/LivingEntity")) {
            for (MethodNode method : classNode.methods) {
                if (("getHealth".equals(method.name) || "m_21223_".equals(method.name)) && method.desc.equals("()F")) {
                    ...
                    method.instructions.add(new MethodInsnNode(Opcodes.INVOKESTATIC, owner, "getHealth", "(Lnet/minecraft/world/entity/Entity;)F", false));
                    ...
                    method.instructions.clear();
                    method.instructions.add(new VarInsnNode(Opcodes.ALOAD, 0));
                    method.instructions.add(new MethodInsnNode(Opcodes.INVOKESTATIC, owner, "getMaxHealth", "(Lnet/minecraft/world/entity/Entity;)F", false));
                    method.instructions.add(new InsnNode(Opcodes.FRETURN));
                    flag = true;
                }
            }
