...
import miku.bai_ze_li.genesis.api.render.tooltip.TooltipBackgroundRenderContext;
...
public class EventHandler {
    static {
        TooltipBackgroundRenderContext.registerTransparentNamespace(Genesis.MOD_ID);
        TooltipBackgroundRenderContext.registerResolver(stack -> {
            if (!(stack.getItem() instanceof Scroll)) {
                return TooltipBackgroundRenderContext.Style.NONE;
            }
            return resolveScrollTooltipStyle(stack);
        });
    }

    private static TooltipBackgroundRenderContext.Style resolveScrollTooltipStyle(ItemStack stack) {
        try {
            ISpellContainer container = ISpellContainer.getOrCreate(stack);
            for (var slot : container.getActiveSpells()) {
                SchoolType schoolType = slot.getSpell().getSchoolType();
                if (schoolType.equals(SpellSchoolRegistry.CHAOS.get())) {
                    return TooltipBackgroundRenderContext.Style.CHAOS;
                }
                if (schoolType.equals(SpellSchoolRegistry.CELESTIAL_SOURCE.get())) {
                    return TooltipBackgroundRenderContext.Style.CELESTIAL_SOURCE;
                }
            }
...
// ... existing code ...
    ...
    public static void onTooltipColor(RenderTooltipEvent.Color event) {
        try {
            TooltipBackgroundRenderContext.Style style = resolveTooltipStyle(event.getItemStack());
    ...
    private static TooltipBackgroundRenderContext.Style resolveTooltipStyle(ItemStack stack) {
        if (stack != null && stack.getItem() instanceof Scroll) {
            return resolveScrollTooltipStyle(stack);
        }
        return TooltipBackgroundRenderContext.resolve(stack);
    }
