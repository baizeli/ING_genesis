...
public class TestBParticle extends Particle {
    ...
    private float spiralSpeed = 0.1F; 
    private float spiralRadius = 0.3F; 

    protected TestBParticle(ClientLevel level, double x, double y, double z) {
        super(level, x, y, z);
        this.setLifetime(500);

        
        initSpiralParameters();
    }

    private void initSpiralParameters() {
        
        this.spiralSpeed = this.random.nextFloat() * 0.2F + 0.1F;

        
        this.spiralRadius = this.random.nextFloat() * 0.4F + 0.1F;

        
        this.spiralAngle = this.random.nextFloat() * (float) (2 * Math.PI);
    }
    ...
    private void updateSpiralRotation() {
        
        float speedFactor = (float) Math.sqrt(xd * xd + yd * yd + zd * zd);
        float adjustedSpeed = spiralSpeed * (1.0F + speedFactor * 1.0F); 

        
        this.spiralAngle += adjustedSpeed;

        
        if (this.spiralAngle > 2 * Math.PI) {
            this.spiralAngle -= 2 * Math.PI;
        }
    }

    public void trail() {
        
        Vec3 spiralOffset = calculateSpiralOffset(this.spiralAngle);
        Vec3 trailAt = new Vec3(this.x + spiralOffset.x, this.y + spiralOffset.y, this.z + spiralOffset.z);
    ...
    private Vec3 calculateSpiralOffset(float angle) {
        
        
        float xOffset = spiralRadius * (float) Math.cos(angle);
        float zOffset = spiralRadius * (float) Math.sin(angle);
        float yOffset = spiralRadius * 0.1F * (float) Math.sin(angle * 0.3F); 
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        float interpolatedSpiralAngle = Mth.lerp(partialTick,
                this.spiralAngle - this.spiralSpeed * 0.5F, 
                this.spiralAngle);

        
        Vec3 currentSpiralOffset = calculateSpiralOffset(interpolatedSpiralAngle);
        double x = Mth.lerp(partialTick, this.xo, this.x) + currentSpiralOffset.x;
        double y = Mth.lerp(partialTick, this.yo, this.y) + currentSpiralOffset.y;
        double z = Mth.lerp(partialTick, this.zo, this.z) + currentSpiralOffset.z;
        ...
        for (int samples = 0; samples < this.sampleSize(); samples++) {
            ...
            float sampleSpiralAngle = interpolatedSpiralAngle - samples * 0.05F; 
            Vec3 sampleSpiralOffset = calculateSpiralOffset(sampleSpiralAngle);

            Vec3 sample = this.getTrailPosition(samples * this.sampleStep(), partialTick);
            
            sample = new Vec3(sample.x + sampleSpiralOffset.x,
                    sample.y + sampleSpiralOffset.y,
                    sample.z + sampleSpiralOffset.z);
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType simpleParticleType, ClientLevel clientLevel,
            ...
            TestBParticle particle = new TestBParticle(clientLevel, x, y, z);
            particle.xd = xSpeed;
            particle.yd = ySpeed;
            particle.zd = zSpeed;
            return particle;
