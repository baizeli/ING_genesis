...
import miku.united_as_one.genesis.common.helper.ColorHelper;
import miku.united_as_one.genesis.common.helper.TrailHelp;
...
public class ThunderArrowRenderer extends EntityRenderer<ThunderArrowEntity> {
    ...
    private static final ResourceLocation TRAIL_TEX_1 = ResourceLocation.tryBuild(Genesis.MOD_ID, "textures/images/trail_thunder.png");
    ...
    @Override
    public void render(ThunderArrowEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
    ...
    private void renderTrail(ThunderArrowEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer) {
        ...
        renderTrailStrip(relativePositions, poseStack, buffer, fullBright, TRAIL_TEX_1, entity.tickCount + partialTicks, false);
        renderTrailStrip(relativePositions, poseStack, buffer, fullBright, TRAIL_TEX_2, entity.tickCount + partialTicks, false);
    ...
    private void renderConeGeometry(VertexConsumer consumer, Matrix4f matrix, Matrix3f normal, Vec3 top, Vec3 bottom, Vec3 perp1, Vec3 perp2, int packedLight, float time, float scale, int sides) {
        ...
        for (int i = 0; i < sides; i++) {
            ...
            Vec3 offset = perp1.scale(cosAngle * scale).add(perp2.scale(sinAngle * scale));
        ...
        for (int i = 0; i < sides; i++) {
            ...
            Vec3 bottom1 = bottomVertices[i];
            Vec3 bottom2 = bottomVertices[nextI];

            float[] topColor = calculateFlowingColor(1.0f, time);
            float[] bottomColor = calculateFlowingColor(0.7f, time);

            float topAlpha = 1f;
            float bottomAlpha = 0f;

            float u1 = (float) i / sides;
            float u2 = (float) nextI / sides;

            TrailHelp.addVertexWithColor(consumer, matrix, normal, top, topColor, topAlpha, 0.5f, 1.0f, packedLight);
            TrailHelp.addVertexWithColor(consumer, matrix, normal, bottom1, bottomColor, bottomAlpha, u1, 0.0f, packedLight);
            TrailHelp.addVertexWithColor(consumer, matrix, normal, bottom2, bottomColor, bottomAlpha, u2, 0.0f, packedLight);
        }
    }

    private void renderTrailStrip(List<Vec3> positions, PoseStack poseStack, MultiBufferSource buffer, int packedLight, ResourceLocation tex, float time, boolean isStellar) {
        if (positions.size() < 2) return;
    ...
    private void renderContinuousTrailPlane(List<Vec3> positions, List<Vec3[]> perpendicularSets, int planeIndex, VertexConsumer consumer, Matrix4f matrix, Matrix3f normal, int packedLight, float time, boolean isShadow) {
        ...
        for (int i = 0; i < positions.size() - 1; i++) {
            ...
            for (int sub = 0; sub < SUBDIVISIONS; sub++) {
                ...
                float alpha1 = TrailHelp.calculateAlpha(subProgress1, isShadow);
                float alpha2 = TrailHelp.calculateAlpha(subProgress2, isShadow);

                alpha1 *= 0.6f;
                alpha2 *= 0.6f;

                float[] color1 = calculateFlowingColor(subProgress1, time);
                float[] color2 = calculateFlowingColor(subProgress2, time);
                ...
                float u1 = subProgress1;
                float u2 = subProgress2;

                TrailHelp.addVertexWithColor(consumer, matrix, normal, bottomLeft, color1, alpha1, u1, 0.0f, packedLight);
                TrailHelp.addVertexWithColor(consumer, matrix, normal, bottomRight, color2, alpha2, u2, 0.0f, packedLight);
                TrailHelp.addVertexWithColor(consumer, matrix, normal, topRight, color2, alpha2, u2, 1.0f, packedLight);
                TrailHelp.addVertexWithColor(consumer, matrix, normal, topLeft, color1, alpha1, u1, 1.0f, packedLight);
            }
        }
    }

    private float[] calculateFlowingColor(float progress, float time) {
        ...
        return TrailHelp.interpolateGradientColor(colorPosition, ColorHelper.THUNDER_THEME);
    }
