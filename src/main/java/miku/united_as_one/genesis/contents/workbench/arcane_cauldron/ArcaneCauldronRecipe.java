...
public class ArcaneCauldronRecipe implements Recipe<ArcaneCauldronRecipe.Input> {
    ...
    public boolean hasRequiredFluids(List<FluidStack> availableFluids) {
        List<FluidStack> remainingFluids = availableFluids.stream().map(FluidStack::copy).collect(java.util.stream.Collectors.toCollection(ArrayList::new));
        for (FluidStack required : fluids) {
            int remainingAmount = required.getAmount();
            for (FluidStack available : remainingFluids) {
                if (FluidHelper.isSameFluidSameComponents(required, available)) {
                    int drained = Math.min(remainingAmount, available.getAmount());
    ...
    public boolean consumesFluid(FluidStack stack) {
        for (FluidStack required : fluids) {
            if (FluidHelper.isSameFluidSameComponents(required, stack)) {
                return true;
            }
        }
        return false;
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipeSerializers.ARCANE_CAULDRON_RECIPE_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return ModRecipeTypes.ARCANE_CAULDRON_RECIPE_TYPE.get();
    }
    ...
    public static class Serializer implements RecipeSerializer<ArcaneCauldronRecipe> {
        ...
        @Override
        public @Nullable ArcaneCauldronRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
