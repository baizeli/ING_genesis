...
import miku.united_as_one.genesis.contents.entity.boss.behavior.*;
import miku.united_as_one.genesis.contents.entity.boss.behavior.bloodbossskill.*;
...
public class BloodBossAi {
    private static final int emergePriority = 150;
    private static final int idleStartPriority = 100;
    private static final int fightStartPriority = 50;
    protected static final ImmutableList<MemoryModuleType<?>> MEMORY_TYPES;
    protected static final ImmutableList<SensorType<? extends Sensor<? super BloodBoss>>> SENSOR_TYPES;
    ...
    static {
        ...
        }
        {
            MEMORY_TYPES=ImmutableList.of(
                    ...
                    MemoryModuleType.HURT_BY_ENTITY,                             // 存储伤害来源实体
    ...
    private static void addFightActivities(Brain<BloodBoss> brain) {
        ...
        // 清除无效目标
        fightBuilder.add(StopAttackingIfTargetInvalid.create(
                livingEntity -> false, (mob, target) -> {}, true));

        // 添加一阶段追击行为：当敌人离开8格范围时追击
        fightBuilder.add(SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.0F));
    ...
    private static void addFightStage2Activities(Brain<BloodBoss> brain) {
        ...
        fightBuilder.add(new GroundSlamBehavior());          // 砸地技能
        fightBuilder.add(new StompBehavior());               // 跺脚技能
        fightBuilder.add(new BloodDaggerSwarmBehavior());    // 七连烈焰飞剑-改
        fightBuilder.add(new BloodDaggerZoneBehavior());     // 三连烈焰场域飞剑-改
